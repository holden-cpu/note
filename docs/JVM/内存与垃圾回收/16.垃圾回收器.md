...
1. 底层优化：JVM优化、GC的变化、元空间、静态域、字符串常量池等

### 垃圾回收器分类

按线程数分（垃圾回收线程数），可以分为串行垃圾回收器和并行垃圾回收器

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616850387823-01e5f72b-bc03-4646-8e4f-825eecd6ee76.png)
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616850552271-3753a51d-4800-42dd-8bf5-9cb6c68f86e2.png)
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616851155582-adb7496b-fc2e-4788-8ea0-cd4cb4b71f7c.png)
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616851135239-00d593fb-f1c9-4e1d-98fb-a42b35d6dec1.png)
...
- 因为如果选择以吞吐量优先，那么必然需要<font color='red'>降低内存回收的执行频率</font>，但是这样会<font color='red'>导致GC需要更长的暂停时间来执行内存回收</font>。
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616852378866-678d3f97-d08f-4816-9455-f632acaaefa7.png)

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616852410516-c09b770f-231d-4d78-a4c2-32888f2999d1.png)

------

**7款经典回收器与垃圾分代之间的关系**

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616852491244-4335f248-3ce5-4181-9ff6-7f5709482c32.png)
...
- ![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616853395736-02f19df0-5f22-424f-a590-682e290f921f.png)
...
- `-XX:+PrintCommandLineFlags`
- ![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616854546520-6135bd66-b111-469a-b9a3-0d2af3a3cd14.png)

- 命令行
- ![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616854617115-f9712386-f9e4-442f-a64e-1fb1218d9332.png)



jdk9

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616854656421-0f9f637f-215f-4927-afb8-e87c7e451cb0.png)

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616854694065-2b405f53-cd6b-42f9-aa87-5f1ef17543ef.png)
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616854978920-599be655-c3ac-41c1-9f90-0e218e9ee6ff.png)
...
![img](https://cdn.nlark.com/yuque/0/2021/png/12731578/1616855771115-98f048dc-7454-4259-9b4a-776e055e46d7.png)
...
- ![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616894889992-b3dd0a79-5e3d-42ba-80bb-0ed9a51d0632.png)
...
在程序中，开发人员可以通过选项"`-XX:+UseParNewGC`"手动指定使用ParNew收集器执行内存回收任务。它表示年轻代使用并行收集器，不影响老年代。（jdk9中此参数被取消）
...
![img](https://cdn.nlark.com/yuque/0/2021/png/12731578/1616895990044-b8f383ad-d9ff-4644-ad81-cd1db3a15bb0.png)
...
- ![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616898193872-074c1a02-9b3d-40ad-a9b5-ab9c8c638c11.png)
...
- ![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616898683859-16f30320-ef37-44e0-b0bd-98c6af65b81b.png)

- 并发清除的时候，用Compact整理内存的话，原来的用户线程使用的内存还怎么用呢？要保证用户线程能继续执行，前提的它运行的资源不受影响嘛。Mark Compact更适合“stop the world”这种场景下使用
...
- CMS收集器无法处理浮动垃圾。可能出现“Concurrent Mode Failure"失败而导致另一次Full GC的产生。在并发标记阶段和清除阶段由于程序的工作线程和垃圾收集线程是同时运行或者交叉运行的，那么在并发标记阶段如果产生新的垃圾对象，CMS将无法对这些垃圾对象进行标记，最终会导致这些新产生的垃圾对象没有被及时回收，从而只能在下一次执行GC时释放这些之前未被回收的内存空间。
...
- - 如果在JDK14中使用XX:`+UseConcMarkSweepGC`的话，JVM不会报错，只是给出一个warning信息，但是不会exit。JVM会自动回退以默认GC方式启动JVM
...
- 官方给G1设定的目标是<font color='red'>在延迟可控的情况下获得尽可能高的吞吐量</font>，所以才担当起“全功能收集器”的重任与期望。
...
- 与此同时，CMS已经在JDK9中被标记为废弃（deprecated）。G1在JDK8中还不是默认的垃圾回收器，需要使用`-XX:+UseG1GC`来启用。
...
- 和之前的各类回收器不同，它同时兼顾年轻代和老年代。对比其他回收器，或者工作在年轻代，或者工作在老年代；
  - ![img](https://cdn.nlark.com/yuque/0/2021/png/12731578/1616917945654-193226de-3f6d-4dbe-9739-6b6d7123841c.png)![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616917954524-1bd46567-c3a8-4601-98c2-6954de7626b4.png)
...
- ![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616920071698-3ea4bea7-b712-46ad-b8e4-738b754feb05.png)
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616920538680-1dab80cf-3def-43fd-8481-c4684823003d.png)
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616922848302-0f246279-26b2-4f77-a412-9795a61d140c.png)
...
- ![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1616923615775-d0810e72-26a8-46c7-a0f3-3ebff888b4a5.png)
...
- YGC时，首先G1停止应用程序的执行（Stop-The-World），G1创建回收集（Collection Set），回收集是指需要被回收的内存分段的集合，年轻代回收过程的回收集包含年轻代Eden区和Survivor区所有的内存分段。![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617089893465-63044836-60ce-4555-9eee-b95b47147fbb.png)
...
- 此阶段，对象树被遍历，Eden区内存段中存活的对象会被复制到Survivor区中空的内存分段，Survivor区内存段中存活的对象。如果年龄未达阈值，年龄会加1，达到阀值会被会被复制到Old区中空的内存分段。如果Survivor空间不够，Eden空间的部分数据会直接晋升到老年代空间。
...
- 对于应用程序的引用赋值语句 oldObject.field（老年代引用）=object（新生代对象），JVM会在之前和之后执行特殊的操作以在dirty card queue中入队一个保存了对象引用信息的card。在年轻代回收的时候，G1会对Dirty Card Queue中所有的card进行处理，以更新RSet，保证RSet实时准确的反映引用关系。
...
- 根区域扫描（Root Region Scanning）：G1 GC扫描Survivor区直接可达的老年代区域对象，并标记被引用的对象。这一过程必须在Young GC之前完成，Young GC会作用到Survivor区。
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617091178034-74aa8027-1b69-4de6-97e1-e25c93be9c37.png)
...
- 要避免Full GC的发生，一旦发生Full GC，需要对JVM参数进行调整。什么时候会发生Full GC呢？比如堆内存太小，当G1在复制存活对象的时候没有空的内存分段可用，则会回退到Full GC，这种情况可以通过增大内存解决。
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617091854880-50622b5f-4c99-4c41-a854-da42be7c1f6e.png)

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617092297217-56172002-0971-46f2-9497-4f8861a7bbee.png)
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617093267349-42b422ee-cbf7-4c5a-a563-7fed360a440b.png)

参数解析

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617093279731-37003b43-66a4-4c37-ad8c-12e793b82fc0.png)

打开GC日志：`-XX:+PrintGCDetails`

输入信息如下

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617093494234-ebfff645-bda4-429b-9b5f-43f7483938a2.png)

参数解析

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617093525275-dcfa71aa-98e3-4a96-9367-11a8f55f110d.png)

打开GC日志：`-XX:+PrintGCTimeStamps -XX:+PrintGCDateStamps`

输出信息如下

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617093650617-56a12c53-6c54-4944-ab02-76da8758a033.png)
...
- 使用G1收集的话，会显示为"garbage-first heap"
...
**Young GC** ![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617094037390-96c4abd2-0b02-4e17-8c01-648119d7ea13.png)

**Full GC**

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617094115517-f954f231-6267-4947-888b-b1990f5032bb.png)
...
![img](https://cdn.nlark.com/yuque/0/2021/png/12731578/1617094369762-b31ed4a0-691b-44d6-a72e-25663643b9ee.png) jdk7

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617094908637-db36c9ec-099b-415e-b753-cafe47472c92.png)

jdk8

4M空间eden区放不下，直接放到老年代

![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617095007753-743d111b-72ff-4527-8eec-2899b3f2dece.png)

**常用日志分析工具**

**保存日志文件**：`-XLoggc:./logs/gc.log`， ./ 表示当前目录，在 IDEA中程序运行的当前目录是工程的根目录，而不是模块的根目录
...
- ![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617095749408-2801beed-20eb-43f5-92be-de64cbe61043.png)
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617096109480-dc8f1675-12d7-4a75-864d-e65721299f02.png)
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617096534462-effe8009-28e7-4a50-b7d8-771989aaa6df.png)



![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617096655645-7a7cb09a-1235-4cf8-8b5a-47448124b3ff.png)
...
![img](https://note-java.oss-cn-beijing.aliyuncs.com/img/1617096861506-d0b2106d-76b3-4236-a1e2-90ce2e471764.png)