...
# 3、mysql配置 /root/mysql/conf/my.cnf
[client]
...
<img src="https://img-blog.csdn.net/20180831173911997?watermark/2/text/aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3pfcnlhbg==/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70" alt="MySQL逻辑架构" style="zoom:150%;" />
...
- `Optimizer`：查询优化器，SQL语句在查询之前会使用查询优化器对查询进行优化。**就是优化客户端请求query**，根据客户端请求的 query 语句，和数据库中的一些统计信息，在一系列算法的基础上进行分析，得出一个最优的策略，告诉后面的程序如何取得这个 query 语句的结果。
  ...
  **For Example**： `select uid,name from user where gender = 1;`这个`select `查询先根据`where `语句进行选取，而不是先将表全部查询出来以后再进行`gender`过滤；然后根据`uid`和`name`进行属性投影，而不是将属性全部取出以后再进行过滤。最后将这两个查询条件联接起来生成最终查询结果。
...
![MySQL逻辑架构](https://note-java.oss-cn-beijing.aliyuncs.com/img/20200801165252510.png)

- 连接层：最上层是一些客户端和连接服务，包含本地sock通信和大多数基于客户端/服务端工具实现的类似于`tcp/ip`的通信。主要完成一些类似于连接处理、授权认证、及相关的安全方案。在该层上引入了线程池的概念，为通过认证安全接入的客户端v提供线程。同样在该层上可以实现基于`SSL`的安全链接。服务器也会为安全接入的每个客户端验证它所具有的操作权限。
...
![image-20210725095412252](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725095412252.png)
...
![image-20210725095805433](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725095805433.png)

![image-20210725095905329](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725095905329.png)
...
![image-20210725113307708](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725113307708.png)
...
![BTree](https://note-java.oss-cn-beijing.aliyuncs.com/img/20200801233134931.png)

![image-20210725123821028](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725123821028.png)
...
如果要查找数据项29，那么首先会把磁盘块1由磁盘加载到内存，此时发生一次IO，在内存中用二分查找确定29在17和35之间，锁定磁盘块1的P2指针，内存时间因为非常短（相比磁盘的I0)可以忽略不计，通过磁盘块1的P2指针的磁盘地址把磁盘块3由磁盘加载到内存，发生第二次I0，29在26和30之间，锁定磁盘块3的P2指针，通过指针加载磁盘块8到内存，发生第三次IO，同时内存中做二分查找找到29，结束查询，总计三次IO。
...
- `const`：表示通过索引一次就找到了，`const`用于比较`primary key`或者`unique`索引。因为只匹配一行数据，所以很快。如将主键置于`where`列表中，MySQL就能将该查询转化为一个常量。
  - ![image-20210725164036168](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725164036168.png)
- `eq_ref`：唯一性索引扫描，读取本表中和关联表表中的每行组合成的一行，查出来只有一条记录。除 了 `system` 和` const` 类型之外, 这是最好的联接类型。
  - ![image-20210725164317676](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725164317676.png)
- `ref`：非唯一性索引扫描，返回本表和关联表某个值匹配的所有行，查出来有多条记录。本质上也是一种索引访问，它返回所有匹配某个单独值的行，然而，它可能会找到多个符合条件的行，所以它应该属于查找和扫描的混合体
  - ![image-20210725165337280](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725165337280.png)
- `range`：只检索给定范围的行，一般就是在`WHERE`语句中出现了`BETWEEN`、`< >`、`in`等的查询。这种范围扫描索引比全表扫描要好，因为它只需要开始于索引树的某一点，而结束于另一点，不用扫描全部索引。
  - ![image-20210725171620447](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725171620447.png)
- `index`：`Full Index Scan`，全索引扫描，`index`和`ALL`的区别为`index`类型只遍历索引树。**也就是说虽然`ALL`和`index`都是读全表，但是`index`是从索引中读的，`ALL`是从磁盘中读取的。**
  - ![image-20210725171720904](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725171720904.png)
- `ALL`：`Full Table Scan`，没有用到索引，全表扫描。
  - ![image-20210725171751576](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725171751576.png)
...
![image-20210725172857315](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725172857315.png)
...
![image-20210725173555370](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725173555370.png)

> rows

`rows`：根据表统计信息及索引选用情况，大致估算出找到所需的记录需要读取的行数。

![image-20210725174007109](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725174007109.png)
...
- `Using temporary`：使用了临时表保存中间结果，MySQL在対查询结果排序时使用了临时表。常见于排序`order by`和分组查询`group by`。**临时表対系统性能损耗很大。**
  ...
  ![image-20210725183051062](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725183051062.png)
...
![image-20210725210621631](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725210621631.png)

![image-20210725210819726](https://note-java.oss-cn-beijing.aliyuncs.com/img/image-20210725210819726.png)
